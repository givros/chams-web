...
async function main(){
  ...
  try{
    ...
    for(let attempt=0;attempt<40;attempt++){const ready=await send('Runtime.evaluate',{expression:'!!document.querySelector("#code") && typeof WebLabCourse!=="undefined" && document.querySelectorAll("[data-step]").length===25',returnByValue:true},appSession.sessionId);if(ready.result?.value)break;await delay(100);}
    const flow=await send('Runtime.evaluate',{awaitPromise:true,returnByValue:true,timeout:90000,expression:`(async()=>{
      ...
      for(let index=0;index<WebLabCourse.course.length;index++){
        ...
        find('#check').click();
        for(let attempt=0;attempt<350&&find('#check').disabled;attempt++)await wait(20);
      ...
      assert(find('#progress-label').textContent==='25 / 25','Progress must be complete');
    ...
    let restored=false;
    for(let attempt=0;attempt<50;attempt++){const check=await send('Runtime.evaluate',{expression:'document.querySelector("#progress-label")?.textContent === "25 / 25" && document.querySelector("#code")?.value.includes("Station météo CHAMS")',returnByValue:true},appSession.sessionId);if(check.result?.value){restored=true;break;}await delay(100);}
    if(!restored)throw new Error('Saved code and progress did not survive reload');
    console.log('Actual browser reload: saved code and 25/25 progress restored.');
