...
async function main(){
  ...
  try{
    ...
    for(let index=0;index<5;index++)for(const variant of ['starter','solution']){
      const code=course[index][variant];const fixture=path.join(directory,'three-'+index+'-'+variant+'.html');
      const scripts=index===4&&variant==='solution'?Object.fromEntries(['three.bundle.js','three-runtime.js'].map(name=>[name,fs.readFileSync(name,'utf8')])):{};
      ...
      if(index===4&&variant==='solution'){
        await evaluate('document.querySelector("#terrain").focus()');
        const key=async(code,type)=>send('Input.dispatchKeyEvent',{type,key:code==='Space'?' ':code,code,windowsVirtualKeyCode:{ArrowRight:39,ArrowLeft:37,ArrowUp:38,ArrowDown:40,Space:32}[code]},sessionId);
    ...
    console.log('Passed: all 5 Three.js solutions and rejection of all 5 incomplete starters.');
